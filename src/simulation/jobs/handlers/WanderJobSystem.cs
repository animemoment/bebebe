...
public sealed class WanderJobSystem
{
    public bool TryAssignJob(int agentIndex, AgentDataPool pool, SimulationContext ctx)
    {
        ...
        // ParallelRng вместо Random.Shared: Shared держит внутренний lock —
        // contention x16 в Phase3a-Parallel (стагглерит случайные батчи).
        if (ParallelRng.NextDouble() > 0.30)
        {
        ...
        int dx = ParallelRng.Next(-2, 3);
        int dy = ParallelRng.Next(-2, 3);
        int tx = curX + dx;
        int ty = curY + dy;
