...
/// <summary>
/// Визуализация содержимого склада: одна иконка на НЕПУСТУЮ клетку StockpileManager.
/// Образец — GroundItemRenderer, отличия: ZIndex 6 (поверх фиолетовой заливки
/// StockpileLayer), иконка 28px (вмещаетя в клетку 64px, визуально отличается от 44px на земле).
/// Данные — только через StockpileManager.SnapshotQueue, прямого доступа к _storage нет.
/// </summary>
public partial class StockpileItemRenderer : Node2D
{
    private const int MaxRenderedInstances = 4096;
    private const float DefaultItemSize = 28f;
    ...
    {
        if (!StockpileManager.Instance.SnapshotQueue.TryDequeue(out var latestSnapshot))
            return;

        while (StockpileManager.Instance.SnapshotQueue.TryDequeue(out var newerSnapshot))
        {
